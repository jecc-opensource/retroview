  ...
  return (
    <div className={styles.modalBox}>
      ...
      <div className={styles.modalBody}>
        ...
        <br />
        <strong>Tech Stack: </strong>{interview.tech_stack.map((tech, index) => {return (<li key={`tech-${index}`} className={styles.techList}>{`${index + 1}. ${tech} `}</li>)})}
        <strong>Résumé: </strong>{interview.resume_version}
        <br />
        <strong>Questions: </strong>{interview.questions.map((question, index) => {return (<li key={`question-${index}`} className={styles.questList}>{`${index + 1}. ${question} `}</li>)})}
